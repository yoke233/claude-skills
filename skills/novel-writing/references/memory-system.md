...
### 上下文策略：子 Agent 协作 + 文件即消息总线

主 agent 不直接加载大量文件，而是通过子 agent 协作完成，agent 之间通过磁盘文件交接：

```
主 Agent（调度员，只读项目索引）
  │
  ├─① 搜集员 Agent
  │    读 10+ 散落文件 → 按「上下文包模板」组装 → 写入磁盘
  │    返回一句话 "上下文包已就绪"
  │
  ├─② 写手 Agent
  │    读 1 个文件（上下文包）→ 逐场景写 → 逐场景 Edit 存盘
  │    返回一句话 "初稿已完成"
  │
  └─③ 审稿人 Agent ×4（并行）
       读章节文件 + 对应教材 → 返回审稿意见
...
**上下文包**（`00-项目/工作流/上下文包-章XXX.md`）是搜集员的产出物，按 `_模板/上下文包模板.md` 结构组装，包含 12 个区域：本章任务、防翻车指令、风格约束、去AI味规则、世界观速查、术语表、出场角色、场景地点、前文衔接、技法约束、按需技法要点、灵感补充（可选）。写手 Agent 只需读这一个文件即拥有完整上下文。

**搜集员内部读取顺序**（主 agent 无需关心）：
1. 并行读取：卷大纲 + 项目总览 + 世界观概述 + 术语表 + 风格指南 + 章节摘要总表 + 已完卷摘要 + 世界观子目录文件（如主 agent 标注了涉及的子领域）
2. 并行读取：出场角色文件（摘要+说话风格+动态状态）+ 地点文件 + 主线活跃情节线
3. 读取去AI味指南 + 按需教材，提取要点

**加载红线**：上下文包总量控制在 ~10000 tokens 以内。如果出场角色超过 5 人，只加载与本章有互动的角色。
...
| 批次 | 操作 | 并行度 |
|------|------|--------|
| Batch 1 | 填章节 frontmatter（字数、出场角色、摘要） | 串行（后续批次依赖） |
| Batch 2 | 章节摘要总表 + 伏笔追踪表 + 情节线更新 + 术语表 | ⚡ 4 文件并行 |
| Batch 3 | 角色动态状态 × N + 时间线 + 卷大纲同步 | ⚡ N+2 文件并行 |
| Batch 4 | 导出正文到 `manuscript/` + 更新项目索引 | ⚡ 2 文件并行 |

写后即录完成后执行**合规审查**（详见 `references/写后即录合规审查.md`）。

### 项目索引

`00-项目/项目索引.md` 是 agent 了解项目全局状态的**唯一入口**。新会话开始时先读此文件，无需逐目录探索。写后即录 Batch 4 自动维护，内容包括：角色/地点/势力清单（含文件路径）、活跃伏笔、活跃情节线、卷进度、最近变更。势力信息记录在 `10-世界观/社会/` 下。

### 工作流追踪文件

`00-项目/工作流/` 目录存放自动生成的工作流追踪文件。每个文件从 `_模板/工作流-*.md` 复制而来，包含 checkbox 清单，agent 逐项执行并标记。状态流转：`待执行 → 进行中 → 通过 / 未通过`。新会话启动时扫描此目录，恢复中断的工作流。
